...
# Null Hypothesis: Female politicians are equally likely
#to support policies female voters want as men.
# Alternative Hypothesis: Female politicians are less likely than men
#to support policies female voters want.
...
# To analyze these variables we first work out the value at the intercept
...
#The coefficient estimate is 14.5
...
# We now need our r value for correlation

cov(data$reserved, data$water)

r <- 2.07



matrixData <- matrix ( c ( data$reserved, data$water))
matrixData
r <- cov(matrixData) [ 1 , 2 ]/ ( sd (matrixData[ , 1 ] ) * sd (matrixData[ , 2 ] ) )

# Now we use this to get the test statistic
...
test_Stat <- (r * sqrt(pmax( n-2) ) / sqrt(pmax (1 - r ^2))
              
test_Stat

##formula won't work here?
